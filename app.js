...
async function api(path, options = {}) {
  if (window.RELAY_BROWSER_API && (location.hostname.endsWith("github.io") || new URLSearchParams(location.search).has("static"))) {
    return window.RELAY_BROWSER_API(path, options);
  }
...
async function initialize() {
  try {
    if (window.RELAY_BROWSER_API && (location.hostname.endsWith("github.io") || new URLSearchParams(location.search).has("static"))) {
      ...
      apiReference.href = "https://github.com/sutasmantas/human-gated-support-automation";
      apiReference.textContent = "View code";
    }
